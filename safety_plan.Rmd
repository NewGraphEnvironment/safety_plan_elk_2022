...
name_project_number <- '2022-056'
# name_project_number2 <- '2021-040'
name_project_name <- 'nupqu-elk-cwf'
name_repo <- 'safety_plan_elk_2022'
...
hotel1 <- '1500 McDonald Ave, Fernie, BC V0B 1M1 - Contact Melanie  1-250-423-8759'



vehicle1 <- '2013 Toyota Tundra black w/flatdeck and yellow can-am quad'
vehicle2 <- '2007 Taoyota Tacoma - gold with canopy'
```
...
The latest version of this pdf can be downloaded [here](https://github.com/NewGraphEnvironment/safety_plan_elk_2022/raw/main/docs/safety_plan_nupqu-elk-cwf.pdf).

<br>

A zip file which includes kml (google earth) and gpx (garmin) files of the sites to be potentially assessed can be downloaded [here](https://github.com/NewGraphEnvironment/safety_plan_elk_2022/raw/main/mapping/sites_nupqu-elk-cwf_20220922_kml_gpx.zip). Georeferenced pdf maps can be accessed and downloaded [here](https://hillcrestgeo.ca/outgoing/fishpassage/projects/elkr/).

<br>

A summary of sites to be potentially assessed is included as Table \@ref(tab:tab-sites).  
...
```{r load-raw, results='hide', eval=F}
# load the target sites from the spreadsheet from mark, turn into a spatial object and burn to the mergin repo
# this is turned to eval=false since we only need to run it once 

crs <- 26911 

sites_raw <- readr::read_csv('data/Site details for Mark.csv') %>% 
  janitor::clean_names() %>% 
  sf::st_as_sf(coords = c("easting", "northing"), crs = crs, remove = F)


# which project?
mergin_name <- 'bcfishpass_elkr_20220904'

# burn the geopackage into the project
sites_raw %>% 
  sf::st_write(paste0(
    '../../gis/mergin/',
    mergin_name,
...
# grab the latest version of the fiss_site and pscis forms and transfer them to our mergin project along with their qmls so they can be styled

# get the forms from where
mergin_project_from <- 'bcfishpass_skeena_20220823'

# list the tables in the mergin projectj
list.files(paste0(
  '../../gis/mergin/',
  mergin_project_from
),
...
# move the latest version of the qml for the pscis_form from  dff-2022 where we now keep it.  
qml <- 'form_pscis.qml'
...
# which project? needed to be redundant becouse last chunk is off
mergin_name <- 'bcfishpass_elkr_20220904'
...
table_sites %>% 
  mutate(name = id,
         desc = paste0(comments, ' ', ktunaxa_response)) %>% 
